...
public:
    Worm(const std::shared_ptr<PlayerState>& initial_state, float ms, TexturesPool& pool) :
            GameActor(initial_state->pos.x, initial_state->pos.y),
    ...
    void update(const std::shared_ptr<PlayerState>& actor_state, unsigned int ms) override {
        position = actor_state->pos;
