...
Battlefield::Battlefield():
        gravity(X_GRAVITY, Y_GRAVITY), world(std::make_unique<b2World>(gravity)) {
...
b2Body* Battlefield::add_body(b2BodyDef& bodyDef) { return world->CreateBody(&bodyDef); }

// ToDo Ver si puedo cambiar el nombre, esto queda como un pasamanos pasando este metodo a game para
// que lo pase a game_loop
void Battlefield::step() { world->Step(TIME_STEP, VELOCITY_ITERATIONS, POSITION_ITERATIONS); }

void Battlefield::destroy_body(b2Body* body) { world->DestroyBody(body); }
...
void Battlefield::clean_dead_entities() { listener->dead_list.clear(); }
